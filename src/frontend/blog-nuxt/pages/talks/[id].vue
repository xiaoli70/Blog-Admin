...
const [{ data: item }, { data: site }] = await Promise.all([
  TalksApi.talkDetail(id),
  AppApi.info(),
]);

const state = reactive({
  commentCount: 0,
  talk: item.value?.data as TalkDetailOutput,
});
...
const cover = computed(() => {
  const arr = site.value?.data?.covers?.talk ?? ["/cover/talk.jpg"];
...
const like = async () => {
  ...
  if (data.value?.succeeded) {
    state.talk.isPraise = data.value.data ?? false;
    state.talk.upvote = data.value.data
      ? state.talk.upvote! + 1
      : state.talk.upvote! - 1;
  }
};
...
useSeoMeta({
  title: "说说详情-" + site.value?.data?.site?.siteName,
