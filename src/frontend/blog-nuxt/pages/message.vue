<template>
  ...
  <div class="message-banner" :style="cover">
    <!-- 弹幕输入框 -->
    <div class="message-container" v-if="site?.data?.site?.isAllowMessage">
...
const cover = computed(() => {
  const arr = site.value?.data?.covers?.message ?? ["/cover/message.png"];
...
useSeoMeta({
  title: "留言-" + site.value?.data?.site?.siteName,
