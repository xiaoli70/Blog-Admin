...
const [{ data: first, error }, { data: last }] = await Promise.all([
  ArticleApi.info(Number(id)),
  ArticleApi.latest(),
]);

if (error.value || !first.value?.data) {
  throw createError({
...
const state = reactive({
  id: id,
  visible: false,
  link: info.value.creationType === 0 ? "" : info.value.link,
  isPraise: info.value.isPraise,
  praiseTotal: info.value.praiseTotal,
});
...
const cover = computed(() => {
  return (
    "background: url(" + info.value.cover + ") center center / cover no-repeat"
  );
});
// 评论数量
...
const textTotal = computed(() => {
  const text = info.value.content?.replaceAll("/<[^>]+>/g, ''", "") ?? "";
...
onMounted(() => {
  state.link = info.value.creationType === 0 ? location.href : state.link;
  nextTick(() => {
    state.visible = true;
    //复制代码
