<template>
  ...
  <div v-if="state.count > 0">
    ...
    <div class="load-wrapper">
      <v-btn outlined v-if="state.pages > (pager.pageNo!)" @click="onMore">
        加载更多...
      </v-btn>
    </div>
  </div>
...
watch(data, () => {
  if (data.value?.succeeded) {
    ...
    if (pager.value.pageNo === 1) {
      state.commentList = data.value?.data?.rows ?? [];
    } else {
      state.commentList.push(...(data.value?.data?.rows ?? []));
    }
  }
});
...
const changeReplyCurrent = async (
  ...
  const { data } = await CommentApi.replyList({
    id: commentId,
    pageNo: current,
    pageSize: 5,
  });
  if (data.value?.succeeded && data.value.data) {
    item!.replyList = data.value.data;
  }
};
