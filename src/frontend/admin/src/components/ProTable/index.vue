...
import { filterEnum, formatValue, handleProp, handleRowAccordingToProp } from './common';
...
// 表格操作 Hooks
const { loading, tableData, pageable, searchParam, searchInitParam, getTableList, search, reset, handleSizeChange, handleCurrentChange } = useTable(
	props.requestApi,
	props.initParam,
	props.pagination,
	props.dataCallback,
...
const colSetting = tableColumns.value!.filter(
	(item) => !['selection', 'index', 'expand'].includes(item.type!) && item.prop !== 'operation' && item.prop !== 'action' && item.isShow
);
...
const print = () => {
	...
	printJS({
		...
		properties: flatColumns
			.value!.filter(
				(item) => !['selection', 'index', 'expand'].includes(item.type!) && item.isShow && item.prop !== 'operation' && item.prop !== 'action'
			)
